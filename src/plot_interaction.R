...
dotplot = function(data_ip,env=NULL,output_path,output_name){
  #adjusted parameters
...
dot_save_as_json = function(data_ip,env=NULL,output_path,output_name){
  # save as json
  env_inter = list()
  interacting_pairs = unique(arrange(data_ip,interacting_pair)$interacting_pair)
  cell_pairs = unique(arrange(data_ip,cell_pairs)$cell_pairs)
  env_inter[["yAxis"]] = unique(arrange(data_ip,interacting_pair)$interacting_pair)
  env_inter[["xAxis"]] = unique(arrange(data_ip,cell_pairs)$cell_pairs)
  env_inter[["value"]] = apply(data_ip, 1, function(x){
...
cells_interaction_heatmap = function(
  ...
  col_func = circlize::colorRamp2(quantile(count_array, probs = c(seq(0,0.6,length.out=10),seq(0.6,1,length.out=90))), viridis(100,option = "D"))
  # draw
...
interaction_plot_save = 
  function(means_path, pvalues_path,output_path,
    ...
    if (!is.null(microenvs_path)){
      ...
      out_json = list()
      out_json[["microenvironment"]] = names(microenvs_cell_pairs)
      for (env in names(microenvs_cell_pairs)){
        ...
        out_json[[env]] = pvalues_means_env %>% 
          dotplot(output_path=output_path,output_name=paste(output_name,env,sep = "_")) %>%
          dot_save_as_json(env = env,output_path=output_path,output_name="inter_dotplot")
      }
      out_json =toJSON(out_json,pretty=F,auto_unbox=T)
      cat(out_json, file = paste(output_path,"/out/json/",output_name,'.json',sep = ''))
    }
...
microenvs_path = file.path(work_dir,'/out/table/microenvironment.csv')
