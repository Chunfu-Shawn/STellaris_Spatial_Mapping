...
traint <- function (st_data, sc_data, st_assay='Spatial', sc_assay='RNA', norm='LogNormalize', nfeatures=2000,
  ...
  sc_st_int <- ScaleData(sc_st_int, features = sc_st_features, verbose = F) %>% RunPCA(features = sc_st_features, verbose = F)
  sc_st_int <- RunUMAP(sc_st_int, dims = 1:50, verbose = F)
...
celltrek_chart <- function (dist_mat, coord_df, spot_dis_intp, dist_cut=500, top_spot=5) {
  ...
  #cat('Pruning graph...\n')
  dist_edge_list_sub <- suppressMessages(dplyr::inner_join(dist_edge_list %>% group_by(Var1) %>% top_n(n=top_spot, wt=-value),
                                          dist_edge_list %>% group_by(Var2) %>% top_n(n=top_spot, wt=-value))) %>% data.frame
  dist_edge_list_sub = dist_edge_list_sub %>% group_by(Var1) %>% slice(which.min(value))
...
knn_cutoff <- function(traint, reduction='umap', k_num=10){
  ## Calculate knn of nearest 10 points
  st_sc_reduction <- traint@reductions[[reduction]]@cell.embeddings
  st_reduction <- st_sc_reduction[traint$type == 'st',] %>% as.data.frame()
  KNN <- dbscan::kNN(st_reduction, k=k_num)
